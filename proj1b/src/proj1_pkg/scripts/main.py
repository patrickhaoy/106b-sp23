...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    try:
        trans = tfBuffer.lookup_transform('base', 'reference/right_gripper_base', rospy.Time(0), rospy.Duration(10.0))
    except Exception as e:
        print(e)
    ...
    if task == 'line':
        ar_tag_trans = tfBuffer.lookup_transform('base', 'ar_marker_0', rospy.Time(0), rospy.Duration(5.0))
        current_ar_tag_position = np.array([getattr(ar_tag_trans.transform.translation, dim) for dim in ('x', 'y', 'z')])
        print("Current AR Tag Position:", current_ar_tag_position)
    
        start_pose = np.concatenate((current_position, [0, 1, 0, 0]))
        end_pose = np.concatenate((current_ar_tag_position, [0, 1, 0, 0]))
        end_pose[2] += .2

        trajectory = LinearTrajectory(5, start_pose, end_pose)
    elif task == 'circle':
        ar_tag_trans = tfBuffer.lookup_transform('base', 'ar_marker_0', rospy.Time(0), rospy.Duration(5.0))
        current_ar_tag_position = np.array([getattr(ar_tag_trans.transform.translation, dim) for dim in ('x', 'y', 'z')])
        end_pose = np.concatenate((current_ar_tag_position, [0, 1, 0, 0]))
